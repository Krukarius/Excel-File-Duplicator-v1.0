...
def openFile():
    ...
    for x in range(2, (int(y))+1):
            newname = "%03d" % x
            directory = filewithoutextension+'-'+newname
            folderpath = os.path.join(file_path, directory)
            subfolderdirectory = folderpath+'/Support files'
            os.mkdir(folderpath)
            os.mkdir(subfolderdirectory)
            save_path = Path(folderpath) / f"{filewithoutextension} - {newname}.{fileextension}"
            wb.save(save_path)
            #wb.save(f''+file_path+filewithoutextension+'- '+newname+'.'+fileextension)
            print (filewithoutextension+'- '+newname+'.'+fileextension)
            print (fileonly+ ' '+filewithoutextension)

rootwindow = tk.Tk()
rootwindow.geometry("500x100")
rootwindow.title("Excel file duplicator v2.0")
font.families()
